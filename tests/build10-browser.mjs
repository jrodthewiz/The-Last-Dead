...
try {
 ...
 await page.goto(`http://127.0.0.1:${server.port}/?debug=1`,{waitUntil:'domcontentloaded'});await page.bringToFront();await page.locator('[data-action="start"]').click({noWaitAfter:true});
 ...
 await page.waitForFunction(()=>window.__DEAD_ARRIVAL__?.renderer?.weaponFX?.rockets?.shell?.count===1,{timeout:2000});
