...
const docsDir = path.resolve('docs/build10');
const gameUrl = process.env.GAME_URL || `http://127.0.0.1:${preview.port}/`;
const debugUrl = `${gameUrl}${gameUrl.includes('?') ? '&' : '?'}debug=1&build10=game`;
...
async function startGame(page) {
  ...
  await page.waitForSelector('#world', { timeout: 60_000 });
...
try {
  ...
  try {
    ...
    await page.screenshot({ path: path.join(docsDir, 'build10-game-desktop.png'), animations: 'disabled' });
    const bellwraith = await injectBellwraithPreview(page);
    await page.screenshot({ path: path.join(docsDir, 'build10-game-bellwraith.png'), animations: 'disabled' });
    await page.evaluate(()=>{window.__DEAD_ARRIVAL__.run.health=10000;});
    ...
    evidence.audioLifecycle=await page.evaluate(async()=>{const a=window.__DEAD_ARRIVAL__.audio,volume=a._volume;await a.resume();a.setMuted(true);await new Promise(r=>setTimeout(r,120));const muted=a._master.gain.value<.001;a.setMuted(false);a.pause();await new Promise(r=>setTimeout(r,50));const paused=a.context.state==='suspended';await a.resume();a.play('ambience');a.play('ambience');const loops=a._voices.get('ambience')?.size??0;a.setVolume(volume);return {muted,paused,resumed:a.isUnlocked,ambientSources:loops,errors:a.loadErrors}});
    evidence.audioSignal=await page.evaluate(async()=>{const a=window.__DEAD_ARRIVAL__.audio;await a.resume();a.stopAmbience();const meter=a.context.createAnalyser();meter.fftSize=1024;a._master.connect(meter);const values=new Float32Array(1024),results=[];for(const type of ['shot','footstep','jump','land','enemy-attack','kill','explosion','slide']){a.play(type,0,{id:Math.random(),cooldown:0});let peak=0;for(let j=0;j<8;j++){await new Promise(r=>setTimeout(r,20));meter.getFloatTimeDomainData(values);for(const v of values)peak=Math.max(peak,Math.abs(v));}results.push({type,peak});}a._master.disconnect(meter);meter.disconnect();a.play('ambience');return results;});
    evidence.sectors=[];
    for(let sector=0;sector<3;sector++){
      await page.evaluate(async index=>{const {newRun,makeCampaignCourse}=await import('/engine.js');const api=window.__DEAD_ARRIVAL__;api.begin();Object.assign(api.run,newRun(makeCampaignCourse(index)));api.run.mode='play';api.run.health=10000;api.run.weapon=3;api.run.pitch=0;api.ui._clearToasts();},sector);
      await page.waitForTimeout(1000);await page.screenshot({path:path.join(docsDir,`sector-${sector+1}.png`)});
      evidence.sectors.push(await page.evaluate(()=>({sector:window.__DEAD_ARRIVAL__.run.sectorId,diagnostics:window.__DEAD_ARRIVAL__.renderer.diagnostics()})));
    }
    await page.evaluate(()=>window.__DEAD_ARRIVAL__.begin());await lockCanvas(page);
    evidence.journey={seconds:0,restarts:0,peakKills:0};
    for(let step=0;step<Number(process.env.JOURNEY_STEPS??24);step++){
      const state=await page.evaluate(()=>({screen:window.__DEAD_ARRIVAL__.screen,kills:window.__DEAD_ARRIVAL__.run.kills}));evidence.journey.peakKills=Math.max(evidence.journey.peakKills,state.kills);
      if(state.screen!=='play'){await page.locator('[data-action="restart"]').first().click();await lockCanvas(page);evidence.journey.restarts++;}
      const key=['KeyW','KeyD','KeyS','KeyA'][step%4];await page.keyboard.down(key);await page.keyboard.press('Space');if(step%3===0)await page.keyboard.press('ShiftLeft');await page.keyboard.press(`Digit${step%4+1}`);await page.mouse.down();await page.waitForTimeout(3000);await page.keyboard.up(key);await page.waitForTimeout(2000);await page.mouse.up();evidence.journey.seconds+=5;
    }
    await page.keyboard.press('Escape');await page.waitForTimeout(100);evidence.journey.pauseScreen=await page.evaluate(()=>window.__DEAD_ARRIVAL__.screen);if(evidence.journey.pauseScreen==='pause')await page.locator('[data-action="resume"]').click();
  ...
  try {
    ...
    await mobilePage.screenshot({ path: path.join(docsDir, 'build10-game-mobile.png'), animations: 'disabled' });
  ...
  await browser.close();
  preview.server.closeAllConnections();await new Promise(resolve=>preview.server.close(resolve));
}
...
await writeFile(path.join(docsDir, 'build10-game-results.json'), JSON.stringify(evidence, null, 2));
console.log(JSON.stringify(evidence, null, 2));
if (!evidence.audioSignal?.every(e=>e.peak>0.00001) || !evidence.audioLifecycle?.muted || !evidence.audioLifecycle?.paused || !evidence.audioLifecycle?.resumed || runtimeErrors.length || !evidence.desktop?.fire?.explosion?.diagnostics?.active || !evidence.desktop?.audio?.loaded || evidence.desktop.audio.decoded < 35 || !evidence.mobile?.touch?.visible) process.exitCode = 1;