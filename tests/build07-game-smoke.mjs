...
const docsDir = path.resolve('docs/build07');
const gameUrl = process.env.GAME_URL || `http://127.0.0.1:${preview.port}/`;
const debugUrl = `${gameUrl}${gameUrl.includes('?') ? '&' : '?'}debug=1&build07=game`;
...
try {
  ...
  try {
    ...
    await page.screenshot({ path: path.join(docsDir, 'build07-game-desktop.png'), animations: 'disabled' });
    const bellwraith = await injectBellwraithPreview(page);
    await page.screenshot({ path: path.join(docsDir, 'build07-game-bellwraith.png'), animations: 'disabled' });
  ...
  try {
    ...
    await mobilePage.screenshot({ path: path.join(docsDir, 'build07-game-mobile.png'), animations: 'disabled' });
...
await writeFile(path.join(docsDir, 'build07-game-results.json'), JSON.stringify(evidence, null, 2));
