...
const docsDir = path.resolve('docs/build08');
const gameUrl = process.env.GAME_URL || `http://127.0.0.1:${preview.port}/`;
const debugUrl = `${gameUrl}${gameUrl.includes('?') ? '&' : '?'}debug=1&build08=game`;
...
try {
  ...
  try {
    ...
    await page.screenshot({ path: path.join(docsDir, 'build08-game-desktop.png'), animations: 'disabled' });
    const bellwraith = await injectBellwraithPreview(page);
    await page.screenshot({ path: path.join(docsDir, 'build08-game-bellwraith.png'), animations: 'disabled' });
    ...
    for(let sector=0;sector<3;sector++){
      await page.evaluate(async index=>{const {newRun,makeCampaignCourse}=await import('/engine.js');const api=window.__DEAD_ARRIVAL__;api.begin();Object.assign(api.run,newRun(makeCampaignCourse(index)));api.run.mode='play';api.run.health=10000;api.run.weapon=3;api.run.pitch=0;},sector);
  ...
  try {
    ...
    await mobilePage.screenshot({ path: path.join(docsDir, 'build08-game-mobile.png'), animations: 'disabled' });
...
await writeFile(path.join(docsDir, 'build08-game-results.json'), JSON.stringify(evidence, null, 2));
