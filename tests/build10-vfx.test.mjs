...
test('flesh contact faces incoming fire and clears the actor centerline',()=>{
 const fx=new ImpactVFX(new THREE.Group());
 // The supplied floor normal is irrelevant to an actor hit at its center.
 fx.burst({id:17,x:0,y:0,z:0,tx:2,ty:1,tz:.4,weapon:1,surface:'flesh',normal:{x:0,y:0,z:1}});
 const splash=fx.splashPool[0],outward=new THREE.Vector3(-2,-.4,-1).normalize();
 const worldNormal=new THREE.Vector3(0,0,1).applyQuaternion(splash.rotation);
 assert.ok(worldNormal.dot(outward)>.99,'spray faces the incoming shot, not the wall behind the actor');
 const offset=splash.position.clone().sub(new THREE.Vector3(8,1.6,4));
 assert.ok(offset.dot(outward)>.3&&offset.length()<.4,'contact clears the body without a detached distant plane');
 fx.update(.05,null);
 assert.ok(fx.splashes.geometry.attributes.life.array[0]>.8,'contact is still readable during the first 50 ms');
 fx.update(.4,null);
 assert.equal(fx.detailActive.splashes,0,'contact clears rather than becoming a floating decal');
});
test('short flesh contact follows a living target instead of being swallowed by its movement',()=>{
 const fx=new ImpactVFX(new THREE.Group()),enemy={x:0,y:1,dead:false};
 fx.burst({id:31,x:0,y:0,z:.4,tx:0,ty:1,tz:.3,weapon:0,surface:'flesh'},true,[enemy]);
 const splash=fx.splashPool[0],offset=splash.position.z-enemy.y*4;
 enemy.y-=.12;fx.update(.05,null);
 assert.ok(Math.abs(splash.position.z-enemy.y*4-offset)<1e-6,'the contact remains ahead of the moving torso');
 fx.update(1,null);assert.equal(splash.target,null,'expired pool slots release actor references');
});
