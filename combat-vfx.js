...
export class CombatVFX{
 ...
 this.beams=make('ColoredBulletTracers',beamGeo,MAX,.85);this.cores=make('TracerWhiteCores',beamGeo,MAX,.9);this.sparks=make('ImpactSparks',new THREE.OctahedronGeometry(1),SPARKS,.9);this.blood=make('BloodVelocityStreaks',new THREE.CylinderGeometry(1,.45,1,4),260,.7);this.blood.material.blending=THREE.NormalBlending;this.blood.material.color.set(0x761217);this.blood.material.toneMapped=true;
 const ringGeometry=new THREE.TorusGeometry(.11,.012,6,18);
 const ringMaterial=new THREE.MeshBasicMaterial({color:0x9ff7ff,transparent:true,opacity:.86,blending:THREE.AdditiveBlending,depthWrite:false,toneMapped:false});
 this.arcRings=new THREE.InstancedMesh(ringGeometry,ringMaterial,ARC_RINGS);this.arcRings.name='ArcDischargeGates';this.arcRings.count=0;this.arcRings.frustumCulled=false;this.arcRings.instanceMatrix.setUsage(THREE.DynamicDrawUsage);this.root.add(this.arcRings);
 this.ringObject=new THREE.Object3D();this.ringColor=new THREE.Color();this.ringDirection=new THREE.Vector3();this.ringPosition=new THREE.Vector3();this.ringRoll=new THREE.Quaternion();
 // Allocate color attributes before shader warmup, including currently empty pools.
 for(const mesh of [this.beams,this.cores,this.sparks,this.arcRings]){mesh.setColorAt(0,this.color.set(0xffffff));mesh.instanceColor.setUsage(THREE.DynamicDrawUsage);}
 this.explosions=new ExplosionVFX(this.root);this.impacts=new ImpactVFX(this.root);this.rockets=new RocketVFX(this.root);
 }
 update(run,dt,gore=true,muzzleSource=null){const reset=this.course!==run.course||run.time<this.lastTime;if(reset){this.particles.length=0;this.seen.clear();this.origins.clear();this.course=run.course;this.explosions?.clear();this.impacts.clear();}this.lastTime=run.time;let count=0,ringCount=0;const current=new Set();
 ...
 this.impacts.update(dt,settings.camera,!!settings.reducedMotion,gore);this.rockets.update(run);
 this.explosions.setCamera(settings.camera||null);this.explosions.update(run,dt,{reducedMotion:!!settings.reducedMotion,gore});
 let b=0;if(gore)for(const p of run.gore||[]){if(b>=260||p.chunk||p.z<=.015)continue;this.object.position.set(p.x*4,p.z*4,p.y*4);this.direction.set(p.vx,p.vz,p.vy);const speed=this.direction.length();this.object.quaternion.setFromUnitVectors(UP,this.direction.normalize());const width=.011+(b%3)*.002;this.object.scale.set(width,Math.min(.22,.038+speed*.095),width);this.object.updateMatrix();this.blood.setMatrixAt(b++,this.object.matrix);}this.blood.count=b;this.blood.instanceMatrix.needsUpdate=true;
 }
}
