...
export class CombatVFX{
 ...
 let b=0;if(gore)for(const p of run.gore||[]){if(b>=260||p.chunk||p.z<=.015)continue;this.object.position.set(p.x*4,p.z*4,p.y*4);this.direction.set(p.vx,p.vz,p.vy);const speed=this.direction.length();this.object.quaternion.setFromUnitVectors(UP,this.direction.normalize());this.object.scale.set(.008,Math.min(.13,speed*.06),.008);this.object.updateMatrix();this.blood.setMatrixAt(b++,this.object.matrix);}this.blood.count=b;this.blood.instanceMatrix.needsUpdate=true;
 }
}
