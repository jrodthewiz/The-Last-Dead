...
import {makeCourse,makeCampaignCourse,newRun,tick,tickPlayer,shoot,shootRifle,shootMelee,cancelMelee,parry,grapple,look,switchWeapon,addPeer,clamp,weapons,castRay,angleDiff} from './engine.js';
...
function processEvents(){
 ...
 for(const e of [...run.events,...(run.peer?.events||[])]){
  ...
  if(session?.role==='host'&&['enemy-attack','explosion','spawn','wave','sector-transition','melee-hit','melee-wall','chainsaw-hit'].includes(e.type)){netEvents.push({...e,seq:++netEventSeq});netEvents=netEvents.slice(-20);}
 }
