...
export const weapons=[{name:'OSSUARY',interval:.22,damage:2,range:20,cone:.008},{name:'BREACH SHOTGUN',interval:.72,damage:1.2,range:7,cone:.15},{name:'ARC LANCE',interval:5,damage:12,range:24,cone:.015}];
...
function damageEnemy(r,e,n,label='HIT',hit=null){if(e.dead)return;e.hp-=n;e.flash=.16;burst(r,e,e.hp<=0?34:12,hit);const d=Math.hypot(e.x-r.x,e.y-r.y,r.z);if(d<1.25&&castRay(r.course,r.x,r.y,Math.atan2(e.y-r.y,e.x-r.x)).dist>d-.15){r.health=Math.min(100,r.health+n*3);r.heal=.5;}
...
const w=weapons[r.weapon];r.cooldowns[r.weapon]=w.interval*(secondary&&r.weapon===0?2:1);r.fireCooldown=r.cooldowns[r.weapon];r.shot=1;r.events.push({type:'shot'});r.repeat=r.lastWeapon===r.weapon?r.repeat+1:0;r.lastWeapon=r.weapon;
 ...
 if(pellets===1||i%2===0)(r.world||r).tracers.push({id:(r.world||r).nextId++,x:r.x,y:r.y,z:oz,tx:r.x+Math.cos(a)*end,ty:r.y+Math.sin(a)*end,tz:clamp(oz+slope*end,.01,1.45),life:.18,duration:.18,weapon:r.weapon,hit:targets.length>0,rail:r.weapon===2});}
