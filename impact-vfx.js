...
export class ImpactVFX{
 ...
 burst(t,gore=true,enemies=null,course=null){
  ...
  this.bitangent.crossVectors(this.tangent,this.normal).normalize();this.basis.makeBasis(this.bitangent,this.tangent,this.normal);splash.rotation.setFromRotationMatrix(this.basis);this.roll.setFromAxisAngle(this.normal,(seed-.5)*.32);splash.rotation.premultiply(this.roll);splash.seed=seed;splash.stretch=.62+seed*.72+(Number(t.weapon)%3)*.08;splash.age=0;splash.duration=.24+seed*.07;splash.size=.48+seed*.14+(Number(t.weapon)%3)*.05;
  ...
  for(let i=0;i<9;i++){
   ...
   q.size=.006+(i%3)*.003;q.spin=a;q.deposit=i%3===0;
  }
 ...
 this.active={marks,smoke};this.detailActive={splashes,drops,deposits:this.deposits?.active||0};}
