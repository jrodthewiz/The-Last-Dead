...
const MAX_MARKS=96,MAX_SMOKE=96,MAX_SPLASH=32,MAX_SPLASH_DROPS=160;
...
function poolMesh(count,name,fragment){
 ...
 const material=isBlood?createBloodSurfaceMaterial({instanced:true}):new THREE.ShaderMaterial({vertexShader:vertex,fragmentShader:fragment,transparent:true,depthWrite:false,side:THREE.DoubleSide,polygonOffset:true,polygonOffsetFactor:-1,polygonOffsetUnits:-1});
 if(isBlood){
  ...
  material.customProgramCacheKey=()=> 'blood-impact-atlas-v2';
 }
...
export class ImpactVFX{
 constructor(parent){this.root=new THREE.Group();this.root.name='SurfaceImpactFX';parent.add(this.root);this.marks=poolMesh(MAX_MARKS,'BallisticScars',markFragment);this.smoke=poolMesh(MAX_SMOKE,'ImpactDust',smokeFragment);this.splashes=poolMesh(MAX_SPLASH,'FleshImpactSplashes',bloodFragment);this.bloodDrops=new THREE.InstancedMesh(new THREE.IcosahedronGeometry(1,1),new THREE.MeshPhysicalMaterial({color:0x39090b,roughness:.29,metalness:0,clearcoat:.18,clearcoatRoughness:.22,transparent:true,opacity:.9,depthWrite:false}),MAX_SPLASH_DROPS);this.bloodDrops.name='FleshImpactDroplets';this.bloodDrops.frustumCulled=false;this.bloodDrops.instanceMatrix.setUsage(THREE.DynamicDrawUsage);this.root.add(this.marks,this.smoke,this.splashes,this.bloodDrops);this.object=new THREE.Object3D();this.normal=new THREE.Vector3();this.forward=new THREE.Vector3(0,0,1);this.shotDirection=new THREE.Vector3();this.tangent=new THREE.Vector3();this.bitangent=new THREE.Vector3();this.basis=new THREE.Matrix4();this.roll=new THREE.Quaternion();this.markCursor=0;this.smokeCursor=0;this.splashCursor=0;this.dropCursor=0;this.markPool=Array.from({length:MAX_MARKS},()=>({position:new THREE.Vector3(),rotation:new THREE.Quaternion(),age:99,duration:8,size:1}));this.smokePool=Array.from({length:MAX_SMOKE},()=>({position:new THREE.Vector3(),velocity:new THREE.Vector3(),age:99,duration:1,size:1,spin:0}));this.splashPool=Array.from({length:MAX_SPLASH},()=>({position:new THREE.Vector3(),rotation:new THREE.Quaternion(),age:99,duration:1,size:.16,stretch:0,seed:0}));this.dropPool=Array.from({length:MAX_SPLASH_DROPS},()=>({position:new THREE.Vector3(),velocity:new THREE.Vector3(),age:99,duration:.8,size:.012,spin:0}));this.clear();}
 clear(){for(const p of [...this.markPool,...this.smokePool,...this.splashPool,...this.dropPool])p.age=99;for(const mesh of[this.marks,this.smoke,this.splashes]){mesh.geometry.attributes.life.array.fill(0);mesh.geometry.attributes.seed.array.fill(0);mesh.geometry.attributes.wet.array.fill(0);mesh.geometry.attributes.stretch.array.fill(0);mesh.geometry.attributes.life.needsUpdate=true;mesh.geometry.attributes.seed.needsUpdate=true;mesh.geometry.attributes.wet.needsUpdate=true;mesh.geometry.attributes.stretch.needsUpdate=true;}this.markCursor=this.smokeCursor=this.splashCursor=this.dropCursor=0;this.bloodDrops.count=0;}
 burst(t,gore=true){
  ...
  this.bitangent.crossVectors(this.tangent,this.normal).normalize();this.basis.makeBasis(this.bitangent,this.tangent,this.normal);splash.rotation.setFromRotationMatrix(this.basis);this.roll.setFromAxisAngle(this.normal,(seed-.5)*.32);splash.rotation.premultiply(this.roll);splash.seed=seed;splash.stretch=.62+seed*.72+(Number(t.weapon)%3)*.08;splash.age=0;splash.duration=.19+seed*.09;splash.size=.2+seed*.12+(Number(t.weapon)%3)*.05;
  for(let i=0;i<9;i++){const q=this.dropPool[this.dropCursor++%MAX_SPLASH_DROPS],a=i*2.399+(Number(t.id)||0),side=Math.sin(a)*.23,spread=Math.cos(a)*.12;q.position.copy(splash.position);q.velocity.copy(this.normal).multiplyScalar(.42+(i%4)*.16).addScaledVector(this.tangent,side).addScaledVector(this.bitangent,spread);q.velocity.y+=.16+(i%3)*.11;q.age=0;q.duration=.45+(i%5)*.11;q.size=.004+(i%3)*.0025;q.spin=a;}
 }
 update(dt,camera,reducedMotion=false,gore=true){let marks=0,smoke=0,splashes=0,drops=0;const stamp=(mesh,p,slot,scale,alpha,wet=0,stretch=0)=>{this.object.position.copy(p.position);this.object.scale.setScalar(scale);this.object.updateMatrix();mesh.setMatrixAt(slot,this.object.matrix);mesh.geometry.attributes.life.setX(slot,alpha);if(mesh.geometry.attributes.seed)mesh.geometry.attributes.seed.setX(slot,p.seed||0);if(mesh.geometry.attributes.wet)mesh.geometry.attributes.wet.setX(slot,wet);if(mesh.geometry.attributes.stretch)mesh.geometry.attributes.stretch.setX(slot,stretch);if(mesh.geometry.attributes.bloodState)mesh.geometry.attributes.bloodState.setXY(slot,1,1-wet);};
 ...
 for(let i=0;i<MAX_SPLASH;i++){const p=this.splashPool[i];p.age+=dt;const alive=p.age<p.duration&&gore;const progress=Math.min(1,p.age/p.duration);this.object.quaternion.copy(p.rotation);stamp(this.splashes,p,i,p.size*(.62+progress*.8),alive?Math.pow(1-progress,1.4):0,alive?1-progress:0,p.stretch);if(alive)splashes++;}
 for(let i=0;i<MAX_SPLASH_DROPS;i++){
  ...
  if(p.position.y<.028){p.age=p.duration;continue;}
  ...
  this.object.scale.set(p.size,p.size*(1+Math.min(2,speed))*fade,p.size);
  this.object.updateMatrix();this.bloodDrops.setMatrixAt(drops++,this.object.matrix);
 }
 this.bloodDrops.count=drops;
 for(const mesh of[this.marks,this.smoke,this.splashes,this.bloodDrops]){mesh.instanceMatrix.needsUpdate=true;for(const key of ['life','seed','wet','stretch','bloodState'])if(mesh.geometry?.attributes?.[key])mesh.geometry.attributes[key].needsUpdate=true;}
