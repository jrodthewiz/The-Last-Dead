...
## Build 10.2 visual correction loop

The next GPT-5.6 Sol review made the remaining priority explicit: the flesh splash needed to be surface-deposited rather than merely a better radial texture. `ImpactVFX` now builds a right-handed tangent basis from the hit normal and incoming shot direction, applies a small deterministic roll, stretches the splash along the projected impact axis, and adds a subdivided wet dome with a short-lived specular glint and drying color response. The existing nine pooled ballistic droplets remain as the volumetric secondary layer; the burst path adds a second tangent spread without allocating per-frame meshes.

The visual harness `tests/build10-blood-visual.mjs` freezes ten seeded impacts on a camera-forward diagnostic plane and writes `docs/build10/blood-depth.png`; the image visibly shows an asymmetric dark-red deposited splash with a directional droplet fan, while its runtime diagnostics report `splashes: 10`, `drops: 90`, and zero page/console errors. The new orientation/depth regression test checks hit-normal alignment, projected incoming direction, animated wetness and finite subdivided vertices. The resize path also keeps the authored `.64` hero weapon scale after a window reflow instead of silently falling back to `.55`.

## Build 10.3 final visual acceptance

The post-change GPT visual review accepted commit `304a224` for the blood/framing increment: the reported deposited splash, displaced wet surface and directional droplet fan address the previous flat-card concern, and `.64` framing now survives resize. It did not justify another geometry, blood or lighting change. Hands/support-hand travel through recoil and Breach's pump stroke remain the highest-value observation checks, while peak explosion readability still needs a real target-visibility frame check; neither is an observed failure in the supplied evidence. The latest smoke run is a readiness signal rather than a locked-60 guarantee (59.38 average, 56 minimum sample), with 77/77 audio decoded and no runtime errors.
