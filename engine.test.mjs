...
test('close-range damage heals and produces bounded blood/gore',()=>{const r=fresh();r.health=40;const e=enemy(r,6.6,6,2);shoot(r);assert.equal(e.dead,true);assert.ok(r.health>40);assert.ok(r.blood.length&&r.gore.length);assert.ok(r.stylePending>0);assert.equal(r.styleTotal,0);for(let n=0;n<40;n++){r.cooldowns[0]=0;enemy(r,6.6,6,2);shoot(r);}assert.ok(r.gore.length<=220&&r.blood.length<=100);});
test('coin toss can be shot for lethal ricochet',()=>{const r=fresh(),e=enemy(r,8,6,7);assert.equal(shoot(r,true),true);assert.equal(r.coinCharges,3);const coin=r.coins[0];r.pitch=Math.atan2(coin.z-eye(r),coin.x-r.x);shoot(r);assert.equal(e.dead,true);assert.ok(r.stylePending>=180);assert.equal(r.styleTotal,0);});
...
test('melee attacks telegraph before damage and punches interrupt the windup',()=>{const r=fresh(),e=enemy(r,6.30,6,20);e.kind=0;e.attack=0;tick(r,1/120,{});assert.equal(e.attacking,true);assert.equal(r.health,100);for(let n=0;n<20;n++)tick(r,1/120,{});assert.equal(r.health,100);parry(r);assert.equal(e.attacking,false);assert.ok(e.stagger>0);for(let n=0;n<30;n++)tick(r,1/120,{});assert.equal(r.health,100);assert.match(r.styleLabel,/INTERRUPT/);});
...
test('bazooka rockets detonate with splash, rocket jump, and wall occlusion',()=>{
 ...
 switchWeapon(r,3);assert.equal(r.weapon,3);assert.equal(weapons[3].kind,'rocket');assert.equal(shoot(r),true);assert.equal(r.projectiles[0].kind,'rocket');for(let n=0;n<80;n++)tick(r,1/120,{});
