...
const browser=await chromium.launch({headless:true,executablePath:process.env.CHROME_PATH||'C:/Program Files (x86)/Google/Chrome/Application/chrome.exe',args:['--disable-background-timer-throttling','--disable-renderer-backgrounding','--disable-backgrounding-occluded-windows']});
...
try{
 ...
 await p.goto(url+'?debug=1',{waitUntil:'networkidle'});await p.screenshot({path:new URL('./docs/menu-desktop.png',import.meta.url).pathname.slice(1)});await p.locator('[data-action="start"]').click();await p.waitForTimeout(200);
