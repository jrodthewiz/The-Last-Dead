...
try{
 ...
 const result=await page.evaluate(async()=>{
  ...
  const gltf=await new GLTFLoader().loadAsync('/art/survivor-meshy/exports/tld-survivor-v02.glb');
  ...
  if(gltf.animations.length!==3)throw new Error('Expected exactly three animation clips, got '+gltf.animations.length);
  ...
  mixer.stopAllAction();mixer.clipAction(gltf.animations.find(c=>c.name==='Idle_Relaxed')).play();mixer.setTime(0);scene.updateMatrixWorld(true);renderer.render(scene,camera);
 ...
 for(const n of ['Idle_Relaxed','Walk_Forward','Run_Forward'])assert.ok(result.clips[n],`Missing ${n}`);
